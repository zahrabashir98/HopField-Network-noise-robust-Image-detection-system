...
# Initial setting
def hopfield(noisePercent,font, train_files, test_files,theta=0.5, time=1000, size=(100,100),threshold=60, current_path=None):
    ...
    for path in test_files:
        ...
        if current_path is not None:
            outfile = current_path+"/output/out_f%s_%s/%s.jpeg"%(font,noisePercent,label[counter])
...
def main(font):
    ...
    train_paths = []
    path = current_path + "/train_pics/train_pics_%s"%(font) + "/"
    for i in os.listdir(path):
        if re.match('[A-Z].jp[e]*g', i):
            train_paths.append(path + i)

    # Second, you can create a list of sungallses file path
    test_paths = []
    for noisePercent in [10,30,60]:
        noisePercent = 10
        path = current_path + "/test_pics/test_pics_%s_%s/" %(font, noisePercent)
        for i in os.listdir(path):
            if re.match('[A-Z]*.jp[e]*g', i):
        ...
        hopfield(noisePercent,font,train_files=train_paths, test_files=test_paths, theta=0.5, time=20000, size=(100, 100), threshold=60,
